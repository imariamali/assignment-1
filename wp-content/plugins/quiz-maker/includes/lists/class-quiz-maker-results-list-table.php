...
class Results_List_Table extends WP_List_Table{
    ...
    public function extra_tablenav( $which ){
        ...
        if( isset( $_GET['filterby'] )){
            $quiz_id = absint( intval( $_GET['filterby'] ) );
        }
        if( isset( $_GET['filterbyuser'] )){
            $user_id = absint( intval( $_GET['filterbyuser'] ) );
        }
        ?>
        <div id="quiz-filter-div-<?php echo $which; ?>" class="alignleft actions bulkactions">
            <select name="filterby-<?php echo $which; ?>" id="bulk-action-quiz-selector-<?php echo $which; ?>">
            ...
            <input type="button" id="doaction-quiz-<?php echo $which; ?>" class="cat-filter-apply-<?php echo $which; ?> button" value="Filter">
        </div>
        <div id="user-filter-div-<?php echo $which; ?>" class="alignleft actions bulkactions">
            <select name="filterbyuser-<?php echo $which; ?>" id="bulk-action-user-selector-<?php echo $which; ?>">
            ...
            <input type="button" id="doaction-user-<?php echo $which; ?>" class="user-filter-apply-<?php echo $which; ?> button" value="Filter">
        </div>
    ...
    public static function get_where_condition(){
        ...
        if(isset( $_REQUEST['fstatus'] )){            
            $fstatus = absint( intval( $_REQUEST['fstatus'] ) );
            switch($fstatus){
                case 0:
                    $where[] = ' `read` = 0 ';
                    break;
        ...
        if( isset( $_REQUEST['filterbyuser'] ) ){
            $user_id = absint( intval( $_REQUEST['filterbyuser'] ) );
    ...
    public function get_bulk_actions() {
        $actions = array(
            'bulk-mark-as-read' => 'Mark as read',
            'bulk-delete' => 'Delete',
        );
    ...
    public function process_bulk_action() {
        ...
        if ( ( isset( $_POST['action'] ) && $_POST['action'] == 'bulk-delete' )
            ...
            $results_ids = esc_sql($_POST['bulk-delete']);
